...
todayBtn.addEventListener('click', () => {
    ...
    title(days[today.getDay()], day, months[month], year);
});
...
eventEnd.addEventListener('input', (e) => {
    ...
    if (e.inputType === "deleteContentBackward") {
        ...
        if (eventEnd.value.length === 0) {
            eventEnd.style.color = 'var(--primary-clr)';
        }
    }
});

addEventBtn.addEventListener("click", (e) => {
    // const currentEventDate = eventDate.innerText.split(' ');
    // const currentEventDate = [parseInt(day, 10), month, parseInt(year, 10)];

    const currentEventName = eventName?.value;
    const currentEventStart = eventStart?.value;
    const currentEventEnd = eventEnd?.value;

    if (eventEnd.value < eventStart.value) {
        eventEnd.style.color = 'red';
    }
    console.log(currentEventDate, currentEventName, currentEventStart, currentEventEnd);
...
document.addEventListener('click', (e) => {
    ...
    if (!dayContainer.contains(e.target) && !e.target.classList.contains('prev-date') && !e.target.classList.contains('next-date')) {
